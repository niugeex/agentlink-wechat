...
本仓库不是腾讯官方的 OpenClaw 微信插件仓库。它的定位是：基于公开可见的微信 iLink / OpenClaw Channel 协议语义，提供一个更适合微信消息接入、示例演示、测试验证和自定义集成的独立 SDK 实现。
...
- 一个面向独立于 OpenClaw 宿主的微信消息接入能力的项目
...
- 本仓库：面向独立于 OpenClaw 宿主的微信消息接入，适用于 AI Agent 集成、自动化助手、业务系统对接和自定义开发
...
- `examples/echo-bot.ts`
  单账号最小示例，用来验证扫码登录、收消息、回消息的基础链路。
- `examples/multi-account-echo.ts`
  多账号基础示例，用来验证已保存账号加载、新账号扫码登录、账号列表查看和多 bot 并行运行。
- `examples/weather-bot.ts`
  天气查询示例，用来展示通过公开天气接口查询城市实时天气。
- `examples/send-media.ts`
  媒体发送示例，用来展示图片、文件、视频的回传能力。
- `examples/openai-doc-agent.ts`
  AI Agent 示例，用来展示微信消息接入本地文档问答 Agent 的方式。
...
- `npm run demo:weather`：运行天气查询示例
- `npm run demo:send-media`：运行媒体发送示例
- `npm run demo:multi-account`：运行多账号基础示例
